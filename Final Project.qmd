...
server <- function(input, output) {
  output$myPlot <- renderPlotly({
    ...
    if (input$position == "Midfielders") {
      ...
      subtitle_text <- "Number of Pass to Shot Creating Chances"
    } else {
      y_axis_col <- ~GA_minus_xGA
      subtitle_text <- "Goals Minus Expected Goals"
    }
    ...
    plot_ly(data = filtered_data, x = ~transfer_fee, y = y_axis_col, color = ~LastName, text = ~team_name, type = "scatter", mode = "markers", marker = list(size = 10, symbol = "circle")) |>
      layout(
        ...
        yaxis = list(title = if(input$position == "Midfielders") "Pass to Shot Creating Chances" else "Goals Minus Expected Goals"),
        legend = list(
          x = 0.2, y = 1.05,
          traceorder = "normal",
          orientation = "h",
        ...
        showlegend = FALSE,
        margin = list(t = 100, l=80) # Increase top margin to push the plot down
      )
  })
}
